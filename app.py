...
#--------------------------------------------------------------------
#class Route:
#    def __init__(self,slug,endpoint,method):
#        self.slug = slug
...
def agregar_producto():
    if request.json.get('codigo') is None:
        return jsonify({"error":"El codigo esta vacio"})
    if request.json.get('descripcion') is None:
        return jsonify({"error":"La descripcion esta vacia"})
    if request.json.get('cantidad') is None:
        return jsonify({"error":"La cantidad esta vacia"})
    if request.json.get('precio') is None:
        return jsonify({"error":"El precio esta vacio"})
...
def eliminar_producto(codigo):
    if codigo is None:
        return jsonify({"error":"El codigo es necesario"})
...
def agregar_carrito():
    if request.json.get('codigo') is None:
        return  jsonify({"error":"El codigo es necesario"})
    if request.json.get('cantidad') is None:
        return jsonify({"error":"La cantidad es necesaria"})
    codigo = request.json.get('codigo')
    cantidad = request.json.get('cantidad')
...
def site_map():
    ...
    for rule in app.url_map.iter_rules():
        ...
        if "GET" in rule.methods:
            url = rule.rule
            res = {'url':url,'endpoint':rule.endpoint,'method':"GET"}
            links.append(res)
            #links.append((url, rule.endpoint))
        if "POST" in rule.methods:
            url = rule.rule
            res = {'url':url,'endpoint':rule.endpoint,'method':"POST"}
            links.append(res)
        if "PUT" in rule.methods:
            url = rule.rule
            res = {'url':url,'endpoint':rule.endpoint,'method':"PUT"}
            links.append(res)
        if "DELETE" in rule.methods:
            url = rule.rule
            res = {'url':url,'endpoint':rule.endpoint,'method':"DELETE"}
            links.append(res)
